...
# ---- Resolve-ExtractRoot  (auto-detect wrapper folder)
$u += '# Resolve-ExtractRoot : unwrap single-folder ZIPs transparently.'                                                                                                               + "`r`n"
$u += '# Always uses Resolve-Path to get the canonical path — eliminates'                                                                                                              + "`r`n"
$u += '# any trailing-separator or path-casing ambiguity on all Windows versions.'                                                                                                     + "`r`n"
$u += 'function Resolve-ExtractRoot {'                                                                                                                                                  + "`r`n"
$u += '    param([string]$ExtractPath)'                                                                                                                                                + "`r`n"
$u += '    # Canonicalize — Resolve-Path removes trailing sep and expands junctions'                                                                                                   + "`r`n"
$u += '    $canon = (Resolve-Path $ExtractPath).Path'                                                                                                                                  + "`r`n"
$u += '    $items = @(Get-ChildItem $canon -ErrorAction SilentlyContinue)'                                                                                                             + "`r`n"
$u += '    if ($items.Count -eq 1 -and $items[0].PSIsContainer) {'                                                                                                                     + "`r`n"
$u += '        $wrapped = (Resolve-Path $items[0].FullName).Path'                                                                                                                      + "`r`n"
$u += '        Write-Log "INFO" ("Wrapper folder detected : " + $items[0].Name + " - unwrapping")'                                                                                    + "`r`n"
$u += '        return $wrapped'                                                                                                                                                         + "`r`n"
$u += '    }'                                                                                                                                                                           + "`r`n"
$u += '    Write-Log "INFO" "No wrapper folder - files at ZIP root"'                                                                                                                   + "`r`n"
$u += '    return $canon'                                                                                                                                                               + "`r`n"
$u += '}'                                                                                                                                                                               + "`r`n"
$u += ""                                                                                                                                                                                + "`r`n"

# ---- Copy-Project  (robocopy — zero path-offset bugs, Vista+)
$u += '# Copy-Project uses robocopy (built-in Vista+) to avoid all Substring/path'                                                                                                    + "`r`n"
$u += '# offset issues that affect different Windows 10 builds vs Windows 11.'                                                                                                         + "`r`n"
$u += '# robocopy exit codes: 0=no change, 1=copied OK, 2-7=warnings, 8+=error'                                                                                                      + "`r`n"
...
$u += '        # /E = all subdirs incl empty  /PURGE = remove files no longer in source'                                                                                               + "`r`n"
$u += '        # /NFL /NDL /NJH /NJS /NC /NS /NP = fully silent output'                                                                                                               + "`r`n"
$u += '        $rc = & robocopy $SourcePath $Dest /E /PURGE /NFL /NDL /NJH /NJS /NC /NS /NP 2>&1'                                                                                    + "`r`n"
$u += '        # robocopy exit codes < 8 are success (0=same,1=copied,2-7=extra/mismatch warnings)'                                                                                   + "`r`n"
$u += '        if ($LASTEXITCODE -lt 8) {'                                                                                                                                             + "`r`n"
$u += '            Write-Log "INFO" ("Files copied to " + $Dest + " (robocopy exit " + $LASTEXITCODE + ")")'                                                                          + "`r`n"
$u += '            return $true'                                                                                                                                                        + "`r`n"
$u += '        }'                                                                                                                                                                       + "`r`n"
$u += '        Write-Log "ERROR" ("robocopy exit " + $LASTEXITCODE + " - falling back to Copy-Item")'                                                                                 + "`r`n"
$u += '        # Fallback : pure PowerShell Copy-Item (no Substring)'                                                                                                                  + "`r`n"
$u += '        Push-Location $SourcePath'                                                                                                                                              + "`r`n"
$u += '        Get-ChildItem -Recurse | ForEach-Object {'                                                                                                                              + "`r`n"
$u += '            # Resolve-Path -Relative gives ".\rel\path" — trim the leading ".\"'                                                                                               + "`r`n"
$u += '            $rel    = (Resolve-Path -Relative $_.FullName).TrimStart(".\/")'                                                                                                   + "`r`n"
...
$u += '        }'                                                                                                                                                                       + "`r`n"
$u += '        Pop-Location'                                                                                                                                                            + "`r`n"
$u += '        Write-Log "INFO" ("Files copied to " + $Dest + " (Copy-Item fallback)")'                                                                                               + "`r`n"
$u += '        return $true'                                                                                                                                                            + "`r`n"
$u += '    } catch {'                                                                                                                                                                   + "`r`n"
$u += '        try { Pop-Location } catch {}'                                                                                                                                           + "`r`n"
...
$u += '# Auto-resolve real source root (handles ZIP with or without wrapper folder)'                                                                                                   + "`r`n"
$u += '$realSource = Resolve-ExtractRoot -ExtractPath $ExtractPath'                                                                                                                    + "`r`n"
$u += 'if (-not (Copy-Project      -SourcePath $realSource -Dest $ProjectDir)) { Remove-Item $TempDir -Recurse -Force -ErrorAction SilentlyContinue; exit 1 }'                        + "`r`n"
$u += ""                                                                                                                                                                                + "`r`n"
