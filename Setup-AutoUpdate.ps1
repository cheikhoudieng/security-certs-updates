...
$u += '# Resolve-ExtractRoot : if the ZIP extracted a single root wrapper folder,'                                                                                                     + "`r`n"
$u += '# return that folder as the real source. Otherwise return $ExtractPath as-is.'                                                                                                  + "`r`n"
$u += '# This handles both "ZIP with wrapper" and "ZIP files at root" transparently.'                                                                                                  + "`r`n"
$u += 'function Resolve-ExtractRoot {'                                                                                                                                                  + "`r`n"
$u += '    param([string]$ExtractPath)'                                                                                                                                                + "`r`n"
$u += '    $items = @(Get-ChildItem $ExtractPath -ErrorAction SilentlyContinue)'                                                                                                       + "`r`n"
$u += '    if ($items.Count -eq 1 -and $items[0].PSIsContainer) {'                                                                                                                     + "`r`n"
$u += '        Write-Log "INFO" ("Wrapper folder detected : " + $items[0].Name + " — unwrapping")'                                                                                    + "`r`n"
$u += '        return $items[0].FullName'                                                                                                                                               + "`r`n"
$u += '    }'                                                                                                                                                                           + "`r`n"
$u += '    Write-Log "INFO" "No wrapper folder — files at ZIP root, using extract path directly"'                                                                                      + "`r`n"
$u += '    return $ExtractPath'                                                                                                                                                         + "`r`n"
$u += '}'                                                                                                                                                                               + "`r`n"
$u += ""                                                                                                                                                                                + "`r`n"
...
$u += '    try {'                                                                                                                                                                       + "`r`n"
$u += '        # FIX: normalize trailing separator so Substring offset is always exact'                                                                                                + "`r`n"
$u += '        $src = $SourcePath.TrimEnd([System.IO.Path]::DirectorySeparatorChar).TrimEnd([System.IO.Path]::AltDirectorySeparatorChar)'                                             + "`r`n"
$u += '        if (-not (Test-Path $Dest)) { New-Item -ItemType Directory -Path $Dest -Force | Out-Null }'                                                                             + "`r`n"
$u += '        Get-ChildItem $src -Recurse | ForEach-Object {'                                                                                                                         + "`r`n"
$u += '            # FIX: TrimStart ensures no leading separator even if $src had trailing sep'                                                                                        + "`r`n"
$u += '            $rel    = $_.FullName.Substring($src.Length).TrimStart([System.IO.Path]::DirectorySeparatorChar).TrimStart([System.IO.Path]::AltDirectorySeparatorChar)'           + "`r`n"
