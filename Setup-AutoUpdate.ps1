...
# ----------------------------------------------------------------
# ----------------------------------------------------------------
#  STEP 2 -- Generate Launcher.ps1 (Remplacement du VBS)
# ----------------------------------------------------------------
Write-Step "2/3" "Generating Launcher.ps1..."

$LauncherPS1 = Join-Path $LauncherDir "Launcher.ps1"

$ps1Content = @"
# ================================================================
# Launcher.ps1 -- Only permanent local file
# Generated by Setup-AutoUpdate.ps1
# ================================================================
...
`$t = [IO.Path]::Combine(`$env:TEMP, 'gh_upd_' + [guid]::NewGuid().ToString('N') + '.ps1')
`$wc.DownloadFile('$BASE_URL/updater.ps1', `$t)
`$wc.Dispose()
...
$psArgs = "-NoProfile -WindowStyle Hidden -ExecutionPolicy Bypass -File `"$LauncherPS1`""
...
try {
    $proc = Start-Process powershell.exe -ArgumentList $psArgs -WindowStyle Hidden -PassThru -Wait
    if ($proc.ExitCode -eq 0) {
        Write-Host "      OK  : First install succeeded!" -ForegroundColor Green
    } else {
        Write-Host ("      WARN: First install -- exit code " + $proc.ExitCode) -ForegroundColor Yellow
        Write-Host ("           Check logs in: " + $LauncherDir) -ForegroundColor Gray
    }
} catch {
    Write-Host ("      WARN: Could not start first run automatically.") -ForegroundColor Yellow
}
