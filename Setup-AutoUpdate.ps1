...
# Path to the ZIP file inside the repository
$REPO_ZIP_PATH  = "security-certs-updates.dat"

# Path to the version file inside the repository
# This file contains a random value generated at each build
# Example content: "a3f8c2d1-9b4e-4f7a-8c1d-2e5f6a7b8c9d"
$REPO_VER_PATH  = "version.txt"

# App folder name inside AppData\Local
$APP_FOLDER     = "Chill"
...
# Logs - number of weekly files to keep
$LOG_KEEP_WEEKS = 8
...
$u += '$ZipUrl          = "' + $ZipUrl + '"'                                                                              + "`r`n"
$u += '$VerUrl          = "' + $VerUrl + '"'                                                                              + "`r`n"
$u += '$LocalVerFile    = "' + (Join-Path $RootDir "version.txt") + '"'                                                   + "`r`n"
...
# Get-RemoteVersion - tiny fetch, just the version.txt
...
$u += '            $wc.Headers.Add("User-Agent", "PowerShell-AutoUpdate/1.0")'                                            + "`r`n"
$u += '            $raw = $wc.DownloadString($VerUrl).Trim()'                                                             + "`r`n"
...
$u += '        Add-Type -AssemblyName System.IO.Compression.FileSystem'                                                   + "`r`n"
...
$u += '        $proc = Start-Process -FilePath $pythonPath -ArgumentList $setupPath -WorkingDirectory $ProjectDir -Wait -PassThru -WindowStyle Hidden' + "`r`n"
...
$u += ""                                                                                                                   + "`r`n"
$u += '# --- Version check (lightweight - just a few bytes) ---'                                                          + "`r`n"
$u += '$remoteVer = Get-RemoteVersion'                                                                                    + "`r`n"
$u += '$localVer  = Get-LocalVersion'                                                                                     + "`r`n"
$u += ""                                                                                                                   + "`r`n"
$u += 'if ($remoteVer -eq $null) {'                                                                                       + "`r`n"
$u += '    Write-Log "WARN" "Could not fetch remote version - skipping update this cycle"'                                + "`r`n"
$u += '    exit 0'                                                                                                         + "`r`n"
$u += '}'                                                                                                                  + "`r`n"
$u += ""                                                                                                                   + "`r`n"
...
$u += ""                                                                                                                   + "`r`n"
$u += 'if (-not (Download-WithRetry -Url $ZipUrl -Dest $ZipPath)) {'                                                     + "`r`n"
$u += '    Remove-Item $TempDir -Recurse -Force -ErrorAction SilentlyContinue; exit 1'                                   + "`r`n"
$u += '}'                                                                                                                  + "`r`n"
$u += 'if (-not (Extract-Zip -ZipFile $ZipPath -OutPath $ExtractPath)) {'                                                + "`r`n"
$u += '    Remove-Item $TempDir -Recurse -Force -ErrorAction SilentlyContinue; exit 1'                                   + "`r`n"
$u += '}'                                                                                                                  + "`r`n"
$u += 'if (-not (Copy-Project -SourcePath $ExtractPath -Dest $ProjectDir)) {'                                            + "`r`n"
$u += '    Remove-Item $TempDir -Recurse -Force -ErrorAction SilentlyContinue; exit 1'                                   + "`r`n"
$u += '}'                                                                                                                  + "`r`n"
$u += ""                                                                                                                   + "`r`n"
...
# ----------------------------------------------------------------
#  STEP 3 - Generate Launcher.vbs
# ----------------------------------------------------------------
Write-Step "3/5" "Generating Launcher.vbs (silent, no window) ..."

$vbs  = 'Dim oShell'                                                                                                                                                                       + "`r`n"
$vbs += 'Set oShell = CreateObject("WScript.Shell")'                                                                                                                                       + "`r`n"
$vbs += 'oShell.Run "powershell.exe -NoProfile -NonInteractive -WindowStyle Hidden -ExecutionPolicy Bypass -File """ & "' + $UpdaterPS1 + '" & """", 0, False'                            + "`r`n"
...
# ----------------------------------------------------------------
#  STEP 4 - Register scheduled task FIRST
# ----------------------------------------------------------------
...
$taskTrigger   = New-ScheduledTaskTrigger -Once -At (Get-Date).AddMinutes($TASK_INTERVAL) -RepetitionInterval (New-TimeSpan -Minutes $TASK_INTERVAL)
$taskSettings  = New-ScheduledTaskSettingsSet -ExecutionTimeLimit (New-TimeSpan -Minutes 30) -RestartCount 3 -RestartInterval (New-TimeSpan -Minutes 5) -StartWhenAvailable -MultipleInstances IgnoreNew
...
# ----------------------------------------------------------------
#  STEP 5 - First update (forced, bypasses version check)
# ----------------------------------------------------------------
Write-Step "5/5" "Running first update now (background, no window) ..."
Write-Info ("Downloading " + $REPO_ZIP_PATH + " and running setup.py ...")
Write-Info "Please wait - this may take 1-2 minutes ..."
Write-Host ""

# Delete local version.txt so the updater is forced to download on first run
$localVerPath = Join-Path $RootDir "version.txt"
if (Test-Path $localVerPath) { Remove-Item $localVerPath -Force }

$proc = Start-Process powershell.exe `
    -ArgumentList ("-NoProfile -NonInteractive -ExecutionPolicy Bypass -File `"" + $UpdaterPS1 + "`"") `
    -WindowStyle Hidden `
    -Wait -PassThru
...
if ($proc.ExitCode -eq 0) {
    ...
    Write-Fail ("First update failed (exit code " + $proc.ExitCode + ")")
